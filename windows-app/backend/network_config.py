...
class NetworkConfigManager:
    ...
    def configure_firewall_rules(self):
        """Configure Windows Firewall rules for the application"""
        try:
            if self.config.get("networkAccess") in ["network", "public"]:
                # Add firewall rules for web interface
                subprocess.run([
                    "netsh", "advfirewall", "firewall", "add", "rule",
                    "name=Blockpanel Web Interface",
                    "dir=in", "action=allow", "protocol=TCP", "localport=1105"
                ], check=True, capture_output=True)
                
                # Add firewall rules for Minecraft servers (25565-25575)
                for port in range(25565, 25576):
                    subprocess.run([
                        "netsh", "advfirewall", "firewall", "add", "rule",
                        f"name=Blockpanel Minecraft Server Port {port}",
    ...
    def remove_firewall_rules(self):
        """Remove firewall rules when switching to local-only mode"""
        try:
            # Remove web interface rule
            subprocess.run([
                "netsh", "advfirewall", "firewall", "delete", "rule",
                "name=Blockpanel Web Interface"
            ], check=True, capture_output=True)
            
            # Remove Minecraft server rules
            for port in range(25565, 25576):
                subprocess.run([
                    "netsh", "advfirewall", "firewall", "delete", "rule",
                    f"name=Blockpanel Minecraft Server Port {port}"
    ...
    def setup_upnp_port_forwarding(self):
        ...
        if not self.config.get("publicAccess", False):
            return False
        
        try:
            # Try using miniupnpc if available
            import miniupnpc
            ...
            if upnp.discover() > 0:
                ...
                result = upnp.addportmapping(1105, 'TCP', local_ip, 1105, 'Blockpanel Web Interface', '')
                if result:
                    print("UPnP port forwarding configured for web interface")
                
                # Add port mappings for Minecraft servers
                for port in range(25565, 25576):
                    upnp.addportmapping(port, 'TCP', local_ip, port, f'Minecraft Server Port {port}', '')
                
                return True
        except ImportError:
            print("miniupnpc not available, skipping UPnP configuration")
        except Exception as e:
...
def apply_network_configuration():
    ...
    if manager.config.get("networkAccess") in ["network", "public"]:
        manager.configure_firewall_rules()
    
    # Setup UPnP if public access is enabled
    if manager.config.get("publicAccess", False):
        manager.setup_upnp_port_forwarding()
